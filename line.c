...
usekreg(f,n)
{
	...
	if (kbdmode == PLAY && kbdplayreg == ukb) {
		mlwrite("[Error: currently executing register %c]",c);
		kbdmode = STOP;
		return FALSE;
	}
...
execkreg(f,n)
{
	...
	if (kbdmode != STOP) {
		mlwrite("[Error: already executing macro]");
		kbdmode = STOP;
		return FALSE;
	}
	...
	if (kp->d_next == NULL) {
		...
		mlwrite("Warning:  only first %d characters will execute",
								KBLOCK);
		i = KBLOCK;
	}
