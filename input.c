		...
		if (typahead()) {
			if ((c = tgetc()) == '[') {
				...
				if (insertmode == REPLACECHAR)
					return abortc;  /* eat the sequence, but return abort */
				if (insertmode) {
					/* remember whether insert or not */
...
int eolchar;
{
	...
	register int expquotef;	/* are we quoting the next expandable char? */
	int firstchar = TRUE;
	...
	for (;;) {
		...
		} else if (expand == EXPAND && expquotef == FALSE) {
