 ...
 *
 * $Header: /usr/build/VCS/pgf-vile/RCS/fakevms.c,v 1.4 1995/08/18 12:32:58 pgf Exp $
 */
#include <sys/types.h>
#include <sys/stat.h>
	...
	const char *path;
{
	...
	if (*temp == SLASHC
	 ...
	 && strchr(temp+1, SLASHC) == 0) {
		...
		mkupper(fake_dev);
		sprintf(fake_dev + strlen(fake_dev), "%s:[000000]", temp+1);
		TRACE((" :: '%s'\n", fake_dev))
		putenv(fake_dev);
	} else {
		is_root_dir = FALSE;
