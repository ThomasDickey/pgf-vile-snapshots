...
ALMOST_EVERYTHING = $(ALLTOOLS) $(ALLHDRS) $(ALLSRC) $(TEXTFILES) $(SHORTSTUFF)
EVERYTHING = $(ALMOST_EVERYTHING) $(BUILTHDRS)
...
rw list-writeable:
	@ls -l $(ALMOST_EVERYTHING) | \
		egrep '^[^l].w' | \
		sed 's;.* ;;'   # strip to last space

no-write:
	chmod -w $(ALMOST_EVERYTHING)
