 ...
 *
 * revision 1.2
 * date: 1991/08/06 15:13:07;
 * sprintf changes
 * 
 * revision 1.1
 * date: 1990/09/21 10:25:01;
	...
	char *fname;
{
	...
	oldumask = umask(0);	/* maximum access allowed to lock files */
	  ...
	  if (*fname != '/')
	   ...
	   pathfmt = "%s/%s";
	  lsprintf(lockname,pathfmt,getpath(fname), LOCKDIR);
	  ...
	  lsprintf(lockname,"%s/%s",lockname,gtname(fname));
...
makelock:  	if (tellall) printf("creating %s\n",lockname);
	  ...
	  else
	  {
		...
		if ((pblk = getpwuid(statblk.st_uid)) == NULL)
		  lsprintf(username,"uid %d",atoi(statblk.st_uid));
		else
